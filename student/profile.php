...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['change_password'])) {
    ...
    $stmt = $conn->prepare("SELECT password FROM tblstudent WHERE student_id = ?");
    ...
    if (password_verify($current_pass, $row['password'])) {
        if ($new_pass === $confirm_pass) {
            if (strlen($new_pass) >= 6) {
                ...
                $update = $conn->prepare("UPDATE tblstudent SET password = ? WHERE student_id = ?");
...
$stmt = $conn->prepare("SELECT s.*, p.program_name, d.dept_name 
                        FROM tblstudent s 
            ...
            <div class="row">
                <div class="col-md-7">
                    <div class="card p-4 shadow-sm border-0 mb-4">
                        <h5 class="fw-bold text-navy border-bottom pb-2 mb-3">Student Information</h5>
                        <form>
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    <label class="form-label text-muted">Student No</label>
                                    <input type="text" class="form-control" value="<?php echo $info['student_no']; ?>" readonly>
                                </div>
                                <div class="col-md-6">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" value="<?php echo $info['program_name']; ?>" readonly>
                            </div>
                             <div class="mb-3">
